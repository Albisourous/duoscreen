...
- `BrowserView.swift` — one pane: `WebStore` (owns the `WKWebView`, `WKNavigationDelegate`, named "A"/"B" for the start page), `WebView` (UIViewRepresentable + scroll delegate for Safari-style chrome auto-hide), `BrowserView` (bottom address bar). No URL persistence — panes always launch on the explainer start page.
